...
parser.add_argument('--num_domain_specific_blocks', default=1, type=int, help='the number of attention blocks in intra-domain sequential encoder in each domain')
parser.add_argument('--num_blocks_mixed_seq', default=1, type=int, help='the number of attention blocks in hybrid-domain sequential encoder')
...
if __name__ == '__main__':
    ...
    if args.inference_only:
        ...
        for domain in datasets_information['domains']:
            print(
                'domain:%s, test (NDCG@%d: %.4f, HR@%d: %.4f, '
                'MRR@%d: %.4f)'
