...
parser.add_argument('--num_domain_specific_blocks', default=1, type=int, help='the number of Mamba layers in intra-domain sequential encoder in each domain')
parser.add_argument('--num_blocks_mixed_seq', default=1, type=int, help='the number of Mamba layers in hybrid-domain sequential encoder')
...
if __name__ == '__main__':
    ...
    for domain in datasets_information['domains']:
        ...
        # val_mrr_10[domain], test_mrr_10[domain] = 0, 0
    ...
    for domain in datasets_information['domains']:
        ...
        df = pd.DataFrame(data=ans_list[domain],
                          columns=['val_NDCG@{0:}'.format(args.top_n), 'val_HR@{0:}'.format(args.top_n), 'val_MRR@{0:}'.format(args.top_n)])
